...
void Phase::Init()
{
	...
	phaseSelected = Phase::GetMaxPhase();
...
void Phase::Paint(HDC hdc, const RECT& rectWindow, StageElement _select_index)
{
	...
	if (_finger_twinkle_cnt % 3 != 0)
	{
		if (phaseSelected < 2)
		{
			oldFont2 = (HFONT)SelectObject(hdc, hFont);
		}
		TextOut(hdc, _fingerPos.x, _fingerPos.y, L"¢º", 1);
	}
...
{
	int maxPhase = Phase::GetMaxPhase();
	if (maxPhase == 3)
	{
		maxPhase = 2;
	}
...
int Phase::GetPhase()
{
	if (phaseSelected >= 2 && Phase::GetMaxPhase() == 3)
	{
		phaseSelected = 3;
	}
