...
void Menu::Paint(HDC hdc, HWND hWnd)
{
	...
	else if (_difficulty)
	{
		...
		SetTextColor(hdc, RGB(255, 255, 255));
		TextOut(hdc, 100, 250, L"[ SELECT DIFFICULTY ]", 21);
		SetTextColor(hdc, RGB(0, 255, 0));
		TextOut(hdc, 200, 300, L"EASY", 4);
		SetTextColor(hdc, RGB(255, 255, 0));
		TextOut(hdc, 200, 380, L"NORMAL", 6);
		SetTextColor(hdc, RGB(255, 0, 0));
		TextOut(hdc, 200, 460, L"HARD", 4);

		_fingerPos.x = 170;
		switch (static_cast<Difficulty>(_finger_difficulty))
		{
...
void Menu::fingerController(const HWND& hWnd)
{
	if (GetAsyncKeyState(VK_UP) & 0x0001)
	{
		...
		{
			if (--_finger_difficulty < 0)
			{
				_finger_difficulty = 0;
			}
		}
	}
	else if (GetAsyncKeyState(VK_DOWN) & 0x0001)
	{
		...
		{
			if (++_finger_difficulty > 2)
			{
				_finger_difficulty = 2;
			}
		}
	}
	else if (GetAsyncKeyState(VK_RETURN) & 0x0001)
	{
		...
		{
		case MI_Menu::start:
			if (_isSetDifficulty == false)
			{
				_difficulty = true;
				_isSetDifficulty = true;
			}
			else
			{
				_difficulty = false;
				SetDifficulty(static_cast<Difficulty>(_finger_difficulty));
