...
public:
	...
	void PlayEffectSound(const EffectSound& effectSound, const float& volume = 0.1f, bool isNewChannel = false);
	void StopEffectSound();

	void PlayBGMSound(const BGMSound& bgmSound, const float& volume = 1.0f, bool isNewChannel = false);
	void StopBGMSound();

	void PlaySkillSound(SkillSound skillSound, const float& volume = 0.1f);
	void StopSkillSound();
